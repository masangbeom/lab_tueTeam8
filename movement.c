...
void setSpeed(int towards, int PWM_Setl, int PWM_Setr) {
	...
	if (PWM_Setl <= 100 && PWM_Setr <= 100) {
		PCA9685_SetOutput(PCA_ADDRESS, LEFT1_TIRE_ENA, 0,
				0xfff * PWM_Setl / 100);
		PCA9685_SetOutput(PCA_ADDRESS, LEFT2_TIRE_ENA, 0,
				0xfff * (PWM_Setl) / 100);
		PCA9685_SetOutput(PCA_ADDRESS, RIGHT1_TIRE_ENA, 0,
				0xfff * PWM_Setr / 100);
		PCA9685_SetOutput(PCA_ADDRESS, RIGHT2_TIRE_ENA, 0,
				0xfff * (PWM_Setr) / 100);
	}
	switch (towards) {
	case FORWARDS:
...
//        case PSB_PAD_RIGHT: setSpeed(TURNRIGHT, 85, 85); delay_ms(30); break;
//        case PSB_PAD_LEFT : setSpeed(TURNLEFT , 85, 85); delay_ms(30); break;
	...
	if (sly < -2) {
		...
		if (slx <= 0) {
			...
			if (slx <= -100)
				...
				setSpeed(BACKWARDS, 30, 40);
			else
				setSpeed(BACKWARDS, 25, 25);
