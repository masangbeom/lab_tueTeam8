...
void printOrderList(char * tb_st, int * commandUI, uint16_t *pos_x, uint16_t *pos_y, int * whereUI) {
...
int printWaitUI(int * commandUI, uint16_t *pos_x, uint16_t *pos_y, int * new_flag, int * whereUI) {
	uint16_t pix_x, pix_y;
	int tableNumber = 0;
	while (*commandUI == 0) {
		...
		if(new_flag[1]) LCD_ShowString(38, 69, "[NEW]" , BLACK, WHITE);

		LCD_ShowString(150, 53, "TABLE 2", BLACK, WHITE);
		if(new_flag[2]) LCD_ShowString(158, 69, "[NEW]" , BLACK, WHITE);

		LCD_ShowString(30, 153, "TABLE 3", BLACK, WHITE);
		if(new_flag[3]) LCD_ShowString(38, 172, "[NEW]" , BLACK, WHITE);

		LCD_ShowString(150, 153, "TABLE 4", BLACK, WHITE);
		if(new_flag[4]) LCD_ShowString(158, 172, "[NEW]" , BLACK, WHITE);

		LCD_ShowString(30, 253, "TABLE 5", BLACK, WHITE);
		if(new_flag[5]) LCD_ShowString(38, 269, "[NEW]" , BLACK, WHITE);

		LCD_ShowString(150, 253, "TABLE 6", BLACK, WHITE);
		if(new_flag[6]) LCD_ShowString(158, 269, "[NEW]" , BLACK, WHITE);
