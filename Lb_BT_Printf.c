	...
	if (flagcnt)
	{
                ...
                for (divisor=Power(10, flagcnt-1); divisor>0; divisor/=10){
                        ...
                        if (leading_zero==TRUE){
                                if (flag0)        print_byte_to_Bluetooth('0');
                                else                print_byte_to_Bluetooth(' ');
                        }
                        else print_byte_to_Bluetooth((char)(result)+'0');
                }
        } else {
                remainder = l;
                ...
                for (divisor=1000000000; divisor>0; divisor/=10){
                        ...
                        if (result!=0 || divisor==1) leading_zero = FALSE;
                        if (leading_zero==FALSE) print_byte_to_Bluetooth((char)(result)+'0');
                }
        }
        return;
}
...
void PrintHex(char *fmt, int l){
        ...
        if (flagcnt){
                if (flagcnt&0x01){        // flagcnt가 홀수 일때, upper를 무시, lower만 출력.
                        ...
                        else print_byte_to_Bluetooth(lHex);
                ...
                for (i=(8-flagcnt)/2; i<4; i++){
                        ...
                        else print_byte_to_Bluetooth(uHex);
                ...
                for (i=0; i<4; i++){
                        ...
                        if (!leading_zero) print_byte_to_Bluetooth(uHex);

                        if (lHex!=0 || i==3) leading_zero = FALSE;
                        if (lHex<10) lHex+='0';
                        else         lHex+='A'-10;
                        if (!leading_zero) print_byte_to_Bluetooth(lHex);
                }
        }
        return;
}
...
{
        if (!fmt || !s) return;
        while (*s) print_byte_to_Bluetooth(*s++);
        return;
}
